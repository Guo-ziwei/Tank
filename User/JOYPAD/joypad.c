...
//#define JOYPAD_LAT PAout(8)  	//锁存     	PB6
//#define JOYPAD_DAT PAin(11)	 	//数据     	PB5  
...
void JOYPAD_Init(void)
{
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOA, ENABLE);	 //使能PB,PD端口时钟
	...
	GPIO_SetBits(GPIOA,GPIO_Pin_11);	//上拉
...
u8 JOYPAD2_Read(void)
{
	...
	{
		temp>>=1;	 
		if(JOYPAD2_DAT==0)temp|=0x80;//LOAD之后，就得到第一个数据
